          ...
          {collections.map((collection) => (
            ...
            >
              <form action={updateCollectionAction} className="grid gap-3">
                <input type="hidden" name="collectionId" value={String(collection._id)} />
                <input name="name" defaultValue={collection.name} />
                <textarea name="description" rows={3} defaultValue={collection.description ?? ""} />
                <input name="coverImageUrl" defaultValue={collection.coverImageUrl ?? ""} />
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <input
                    name="publishAt"
                    type="datetime-local"
                    defaultValue={collection.publishAt ? new Date(collection.publishAt).toISOString().slice(0, 16) : ""}
                  />
                  <input
                    name="unpublishAt"
                    type="datetime-local"
                    defaultValue={collection.unpublishAt ? new Date(collection.unpublishAt).toISOString().slice(0, 16) : ""}
                  />
                </div>
